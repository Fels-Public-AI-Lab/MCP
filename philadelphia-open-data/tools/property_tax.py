"""
Philadelphia Property Assessment & Census Data tools.

Migrated from Philadelphia_Property_Tax_Fairness.py (metadata-driven version).

Privacy note: owner name is intentionally excluded as a search parameter
across all tools in this module. While owner_1 and owner_2 are public OPA
fields and are returned in results, allowing owner-name search would make it
trivial to enumerate all properties held by an individual or entity — a
pattern more useful for surveillance than for the civic/policy analysis this
server is designed to support. Users who need owner-based lookups should
query the OPA dataset directly via carto_query.
"""
...
class PropertySearchInput(BaseModel):
    ...
    NOTE: owner_name is intentionally not a search parameter. See module
    docstring for the privacy rationale. owner_1/owner_2 are still returned
    in results so callers can identify the owner of a property they located
...
def _calculate_prb(assessed_values: List[float], sale_prices: List[float]) -> float:
    ...
    Returns β coefficient. Values near 0 indicate no bias, negative indicates regressivity,
    positive indicates progressivity.
    """
    import math
    ...
    try:
        ...
        y_values = [math.log((av/sp) / 1.0) for av, sp in valid_pairs]
...
async def philly_search_properties(params: PropertySearchInput) -> str:
    ...
    Owner name (owner_1, owner_2) is returned in results but is not a search parameter.
    """
    try:
        conditions = []
    ...
    try:
        ...
        if params.address or params.parcel_number:
            ...
            if subject_result["rows"]:
                ...
                return "Subject property not found. Please verify address or parcel number."
